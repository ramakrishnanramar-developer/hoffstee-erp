...
const NoteVoucherList = () => {
    ...
    const loadVouchers = async () => {
        try {
            loadPermissions();
            const res = await GetVouchersList(STRINGS.Codes.NoteVoucher);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            toast.success('Credit/Debit Note deleted!');
    ...
    const handleEdit = (voucherId) => {
        // Redirect to Credit/Debit Note page with id query param
        window.location.href = `/note?id=${voucherId}`;
    };

    return (
        <div className="modules-page">
            <h2>🧾 Credit/Debit Note Vouchers</h2>
            {permissions.isView && (
...
export default NoteVoucherList;
