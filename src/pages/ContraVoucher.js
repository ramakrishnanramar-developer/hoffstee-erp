...
const ContraVoucher = () => {
    ...
    const GenerateCode = async (id) => {
        try {
            const data = await GetVoucherCode(id, STRINGS.Codes.ContraVoucher);
            setFormData((prev) => ({
                ...prev,
    ...
    const loadPermissions = async () => {
        try {
            const data = await getPermissionsByPage(STRINGS.PAGES.ContraVoucher);
    ...
    return (
        <div className="modules-page">
            <h2>🧾 {editingId ? "Edit" : "Create"} Contra Voucher</h2>
